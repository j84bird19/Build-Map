# Cabin Rebuild Mapper v2.9

Flat, GitHub Pages-ready build.

## v2.9 fixes
- New objects and uploaded images are assigned to the currently active layer.
- The active layer is clearly highlighted and marked ACTIVE.
- Layer opacity is visible and adjustable from 0–100%.
- Upload Image uses the device's native file picker and adds the image to the active layer.
- Projects panel includes project naming, Save Project, Load Project, New Blank Project, Clear Current, saved-project list, and delete controls.
- The top Save button opens the Projects panel so project naming and save destination are always visible.
- Existing locked editor, boolean, zoom, rotate, resize, duplicate, custom-shape, parts inspector, and color-picker systems were preserved.


## v3.1 changes
- Added independent per-object opacity in the Parts / Properties Inspector.
- Layer opacity now multiplies each object's stored opacity instead of overwriting it.
- Strengthened the active layer indicator with a thick accent border, left bar, tinted background, and `✓ ACTIVE` badge.
- Existing editing, boolean, zoom, sizing, project, upload, and layer systems remain unchanged.


## v3.1
- Added a clearly labeled **Selected object opacity** control directly inside the Parts / Properties Inspector.
- The slider updates the selected object live while preserving separate layer opacity.

## v3.2
- Locked layer system remains unchanged.
- Selected-part properties now update live from the Parts Inspector.
- Name, label, dimensions, depth, units, layer, material, color, opacity, and notes no longer require an Update button.
- The selected-part button now simply closes the inspector after live edits.
